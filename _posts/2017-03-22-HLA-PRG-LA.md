...
HLA\*PRG:LA approximates the graph alignment process by starting with linear sequence alignments. It brings down the resource requirements per sample for the HLA typing process to 30G RAM/30 CPU hours, and produces highly accurate calls.
...
![](/download/HLPRGLA_Figure_PRG.png "An an example PRG with 3 haplotypes.")
...
![](/download/HLPRGLA_MSAandPRG.png "A multiple sequence alignment and the corresponding fully recombining PRG")

To align a read pair, we go through the following steps (see below for descriptions of the steps):

![](/download/HLPRGLA_FullInferenceProcess.png "Graph alignment by linear approximation")
...
![](/download/HLAPRGLA_performance.png "Accuracy HLA\*PRG and HLA\*PRG:LA")
